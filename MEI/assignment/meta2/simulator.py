...
class Simulator:
    ...
    def run(self):
        print("pid,arrival_time,cpu_bursts_time,io_bursts_time,bursts_time,tat,ready_wait_time,response_time,io_wait_time", file = self.ofile)
        self.env.run()
        self.ofile.close()
...
class ProcessComponent(sim.Component):
    ...
    def process(self):
        ...
        #print("PID: ",self.pid,"Arrival: ",self.arrival,"Finish: ",self.simulator.env.now(), "teste: ",self.response_time)
        ...
        print(self.response_time-self.arrival, end = ",", file = self.simulator.ofile)
    ...
    def __schedule_cpu_burst(self, burst):
        if self.simulator.cpu_scheduler == "fcfs":
            ...
            if self.response_time == -1:
                    self.response_time = self.simulator.env.now()
