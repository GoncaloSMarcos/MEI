    ...
    def __schedule_cpu_burst(self, burst):
        if self.simulator.cpu_scheduler == "fcfs":
            ...
            while True:
                ...
                if self.isbumped(): # This fixes the need for standby below
                    continue
                yield self.hold(duration = s, mode = "")
                if not self.isbumped():
                    break
                s -= self.simulator.env.now() - self.mode_time()
                # yield self.standby() # This does not always work as intended
            self.release(self.simulator.cpu)
        else:
...
if __name__ == '__main__':
    ...
    cpu_bursts_sum = sum([sum(p.bursts[0::2]) for p in processes])
    cpu_claimers_length = sum(simulator.cpu.claimers().length_of_stay.x())

    if abs(cpu_claimers_length / cpu_bursts_sum - 1) > 1e-10:
        print("Warning: Possible BUG detected. Please report.")
