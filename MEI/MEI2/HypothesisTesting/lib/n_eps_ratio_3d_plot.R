...
yBubble = resultBubble$X.Ratio.of..Comp.failure.no.failure
yInsert = resultInsert$X.Ratio.of..Comp.failure.no.failure
yMerge = resultMerge$X.Ratio.of..Comp.failure.no.failure
yQuick = resultQuick$X.Ratio.of..Comp.failure.no.failure
...
# Insert Sort scatter
resultCut = subset(quickDataB,MaxR>N/3) # para nao termos os dados de MAX_R = N/3 QUALQUER COISA METER 2.5
resultCut = subset(resultCut,MaxR<N-15) # para nao termos os dados de N = MAX_R


p <- plot_ly(resultCut, x = resultCut$N, y =  resultCut$Eps, z =  resultCut$X.Comparations.w..Failure, color = "blue", opacity=0.19, size = I(10)) %>%
  add_markers() %>%
  layout(scene = list(xaxis = list(title = 'N'),
                      yaxis = list(title = 'EPS'),
                      zaxis = list(title = 'Comp failure')))
